# MKBOOTIMG
chmod a+x device/samsung/a04s/prebuilt/mkboot/mkbootimg
...
export FOX_TARGET_DEVICES="a04s, a04sub, SM-A047M, SM-A047F"
export TARGET_DEVICE_ALT="a04s, a04sub, SM-A047M, SM-A047F"
export FOX_BUILD_DEVICE="a04s, a04sub, SM-A047M, SM-A047F"

# Function to export build variables
export_build_vars() {
    echo -e "${GREEN}Exporting build vars from the a04s tree${RESET}"
    ...
    export FOX_USE_SPECIFIC_MAGISK_ZIP="$PWD/device/samsung/a04s/prebuilt/su/APatch.apk"
...
set_env_var() {
    ...
    echo " export CURR_DEVICE=a04s"
    exit 1
}

# Function to handle mismatched CURR_DEVICE variable
var_not_eq() {
    echo -e "${RED_BACK}CURR_DEVICE not equal to a04s${RESET}"
    ...
    echo -e "${RED}Skipping a04s specific build vars...${RESET}"
}
